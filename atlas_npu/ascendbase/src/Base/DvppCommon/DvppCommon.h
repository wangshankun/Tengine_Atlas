...
struct VdecCallBackUserData {
    uint32_t channelId;
    uint64_t frameId;
    MessageQueue<std::shared_ptr<DvppDataInfo>>* outputQueue;
    uint32_t width;               // Width of image
    uint32_t height;              // Height of image
    uint32_t widthStride;         // Width after align up
    uint32_t heightStride;        // Height after align up
    acldvppPixelFormat inFormat;  // Format of image
};

struct DeviceStreamData {
    // std::shared_ptr<StreamData> imgOrigin;
...
public:
    ...
    static APP_ERROR GetVideoDecodeStrideSize(uint32_t width, uint32_t height, acldvppPixelFormat format,
                                              uint32_t &widthStride, uint32_t &heightStride);
    APP_ERROR GetVideoDecodeDataSize(uint32_t width, uint32_t height, acldvppPixelFormat format, uint32_t &vdecSize);
    ...
    APP_ERROR CombineVdecProcess(std::shared_ptr<DvppDataInfo> data, VdecCallBackUserData *userdata);
